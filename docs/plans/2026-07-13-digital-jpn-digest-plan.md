...
- heroImage は閣僚等の肖像を含む YouTube サムネイル(i.ytimg.com)ではなく、記事専用にユーザーから渡されたデザイン画像(`public/article/{slug}.png`)があればそれを、なければ全ダイジェスト記事共通の固定 SVG(`/article/digest-press-conference.svg`)を使う(肖像権・パブリシティ権・政治的中立性への配慮。2026-07-13 追加変更、同日中に記事専用画像対応へ再変更)
...
3. **記事生成**(動画ごと):
   ...
   - `src/lib/articles/registry.ts` の `articles` 配列にエントリを追加:
     ...
     - `heroImage`: 記事生成前に、記事専用のサムネイル画像を用意するか **ユーザーに確認する(Ask First)**。渡された場合は元の形式に関わらず PNG に変換して `public/article/{slug}.png` として保存し(`magick <入力> public/article/{slug}.png`)、`/article/{slug}.png` を使う。推奨アスペクト比は `ArticleLayout` の `aspect-ratio: 2/1` に合わせた 2:1(大きくずれる場合はロゴ・タイトルが上下の余白内に収まるか確認する)。300KB を超える場合は `pngquant --quality=60-85 --force` 等で他記事画像と同程度のサイズまで圧縮する。渡されなかった場合は既定の共通サムネイル `/article/digest-press-conference.svg` を使う。いずれの場合も閣僚等の肖像を含む YouTube サムネイルは肖像権・パブリシティ権・政治的中立性の観点から使用しない
